...
void pcm_submit() {
    ...
    if (audio_mute) {
        ...
        for (int i = 0; i < AUDIO_BUFFER_LENGTH; i++) {
            /* mix left & right */
            audiobuffer_dma[i + offset] = (snd.output[0][i] + snd.output[1][i]) >> 1;
        }
    }
}

uint8_t fb_buffer[SMS_WIDTH * SMS_HEIGHT]  __attribute__((section (".itcram_data")));
    ...
    while (true)
    {
        ...
        if (drawFrame)
        {
            ...
            if (delta >= 1000) {
                ...
                printf("SND: %d %d\n", snd.sample_count, AUDIO_BUFFER_LENGTH);
                frames = 0;
